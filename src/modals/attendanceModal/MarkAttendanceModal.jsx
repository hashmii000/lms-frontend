...
export default function MarkAttendanceModal({ open, onClose }) {
  ...
  useEffect(() => {
    ...
    getRequest(`sections?classId=${classId}&isPagination=true`)
  ...
  const applyHandler = async () => {
    ...
    try {
      ...
      const enrollRes = await getRequest(
        `studentEnrollment?session=${sessionId}&currentClass=${classId}&currentSection=${sectionId}&isPagination=true`
      );
  ...
  const markAllAs = (status) => {
    ...
    message.success(`All students marked as ${status === 'P' ? 'Present' : status === 'A' ? 'Absent' : 'Holiday'}`);
  };
  ...
  const columns = [
    {
      ...
      width: 90,
      align: 'center',
      render: (text) => <span className="font-semibold text-gray-700 bg-gray-50 px-2 py-1 rounded">{text}</span>
    },
    {
    ...
    },
    {
      ...
      render: (_, record) => (
        <Radio.Group
          ...
          onChange={(e) =>
            handleAttendanceChange(record.studentId, e.target.value)
          }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 ">
              ...
              <div>
                ...
                <p className="text-sm font-[400] text-gray-500 m-0">Record student attendance for today</p>
              </div>
            </div>
        ...
        <Spin spinning={studentsLoading} tip="Loading students...">
          ...
          {students.length > 0 && (
            <div className="flex items-center justify-between mb-3 px-2">
              ...
              <div className="text-sm text-gray-600">
                ...
                Present: <b className="text-green-600">{stats.present}</b> |
                Absent: <b className="text-red-500">{stats.absent}</b> |
          ...
          {students.length === 0 && !studentsLoading ? (
            <div className="text-center py-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
              ...
              <p className="text-sm text-gray-400">Please select filters and click Apply to load students</p>
            </div>
          ) : (
            <Table
              columns={columns}
...
.ant-radio-group-solid :where(.css-dev-only-do-not-override-1odpy5d).ant-radio-button-wrapper-checked:not(.ant-radio-button-wrapper-disabled) {
    color: #fff;
    background: #0c3b73;
    border-color: #0c3b73;
}
