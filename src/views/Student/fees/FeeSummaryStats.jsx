...
export default function FeeStats({ ledgerData = [] ,loading}) {
  // ✅ Safe calculation using useMemo
  const { totalAmount, totalPaid, totalDue } = useMemo(() => {
    const amount = ledgerData.reduce((sum, m) => sum + (m.totalAmount || 0), 0)
    const paid = ledgerData.reduce((sum, m) => sum + (m.totalPaid || 0), 0)
    const due = ledgerData.reduce((sum, m) => sum + (m.totalDue || 0), 0)
