...
import SectionMasterModal from './SectionMasterModal'
...
const SectionMaster = () => {
  ...
  const handleDragEnd = (event) => {
    ...
    setData((prev) => {
      ...
      return reordered.map((item, index) => ({
        ...item,
        order: (page - 1) * limit + index + 1, // ✅ GLOBAL ORDER
      }))
    })
  }
  ...
  const saveSectionOrder = async () => {
    try {
      ...
      const classId = data[0]?.class?._id // assuming same class
      ...
      const res = await putRequest({
        url: 'sections/updateSectionOrder',
        cred: payload,
      })

      toast.success(res?.data?.message || 'Section order updated successfully')
  ...
  useEffect(() => {
    ...
    getRequest(`sections?${params}`)
      .then((res) => {
        console.log('session', currentSession._id)

        const responseData = res?.data?.data
        setData(responseData?.sections || [])
        setTotal(responseData?.totalSections || 0)
      })
      .catch((error) => toast.error(error?.response?.data?.message || 'Failed to fetch sections'))
      .finally(() => setLoading(false))
  }, [page, limit, searchTerm, classId, updateStatus, currentSession])
  ...
  const confirmDelete = () => {
    ...
    deleteRequest(`sections/${selectedItem._id}`)
      .then((res) => {
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-xs sm:text-sm text-gray-500">
              Manage class sections and their status
            </p>
          </div>
